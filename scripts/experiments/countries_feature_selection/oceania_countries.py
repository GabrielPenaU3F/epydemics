...
from scripts.experiments.countries_feature_selection.useful_functions import get_continent_dataframe, \
    split_dataframes_from_continent, find_peak_location

dfs = split_dataframes_from_continent('Oceania')
...
for df in dfs:
    ...
    if peak_xs.size != 0:  # check if it's empty
        ...
        output_df = output_df.append({'country': country, 'peak_date': max_peak_date, 'peak_magnitude': max_peak_value},
                                     ignore_index=True)
        fig_filename = 'E:/Universidad/Investigación/Coronavirus/Primer pico de países del mundo/figs/Oceania/fig_' \
                       + str(country) + '.png'
        fig.savefig(fig_filename)

print('Number of countries with peak: ' + str(df_count))
print(output_df)
output_df.to_csv('E:/Universidad/Investigación/Coronavirus/oceania_first_peaks.csv')
