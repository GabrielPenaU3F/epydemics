...
def get_full_dataframe():
    ...
    full_df = DataManager.get_full_data()
    return full_df[['location', 'date', 'new_cases', 'new_deaths']]
...
def split_dfs_by_country():
    full_df = get_full_dataframe()
    dfs = []
    for country in full_df['location'].unique():
        country_df = full_df[full_df['location'] == country]
        country_df = country_df[country_df['new_cases'].notna()]
        country_df['new_deaths'] = country_df['new_deaths'].fillna(0)
        country_df = country_df.reset_index()
        if not country_df.empty:
