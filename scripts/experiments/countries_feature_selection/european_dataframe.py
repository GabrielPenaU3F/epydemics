...
dfs = split_dataframes_from_continent('Europe')
...
for df in dfs:
    ...
    if peak_xs.size != 0:  # check if it's empty
        ...
        output_df = output_df.append({'country': country, 'peak_date': max_peak_date, 'peak_magnitude': max_peak_value},
                                     ignore_index=True)
        fig_filename = 'E:/Universidad/Investigación/Coronavirus/Primer pico de países del mundo/figs/Europe/fig_' \
                       + str(country) + '.png'
        fig.savefig(fig_filename)

print('Number of countries with peak: ' + str(df_count))
print(output_df)
output_df.to_csv('E:/Universidad/Investigación/Coronavirus/europe_first_peaks.csv')
