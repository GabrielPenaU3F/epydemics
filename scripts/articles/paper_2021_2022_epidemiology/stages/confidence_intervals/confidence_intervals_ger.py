...
mtbi_unit = 'sec'
filename = 'E:/Universidad/Investigación/Coronavirus/Python/script_outputs/ger_mtbi_confidence.pdf'
...
for day in range(1, len(datasets_ger) + 1):
    ...
    inf95, sup95 = stats.t.interval(alpha=.95, df=len(dataset_ger) - 1,
                                    loc=np.mean(dataset_ger), scale=stats.sem(dataset_ger))
    infs95.append(inf95)
    sups95.append(sup95)

    inf99, sup99 = stats.t.interval(alpha=.99, df=len(dataset_ger) - 1,
                                    loc=np.mean(dataset_ger), scale=stats.sem(dataset_ger))
    infs99.append(inf99)
    sups99.append(sup99)

avg_mtbi = np.array([np.mean(ith_day_mtbi) for ith_day_mtbi in datasets_ger])
std_mtbi = np.array([np.sqrt(np.var(ith_day_mtbi, ddof=1)) for ith_day_mtbi in datasets_ger])
