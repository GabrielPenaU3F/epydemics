...
class CalculateMTBIOWIDTest(unittest.TestCase):
    ...
    def test_mtbi_owid_arg_exact(self):
        expected_mtbis_day = np.array([633, 640, 610, 551, 595, 594, 607, 615, 625, 594, 630])
        mtbis_day = np.round(86400 * np.array(Fitter.calculate_mtbis('Argentina', dataset='total_cases',
                                                                     start=1, end=40, start_from=30, unit='day',
                                                                     fit_x0=(0.1, 1), formula='exact_conditional')))
        testing.assert_array_equal(expected_mtbis_day, mtbis_day)
    ...
    def test_mtbi_owid_arg_approx(self):
        ...
        mtbis_day = np.array(Fitter.calculate_mtbis('Argentina', dataset='total_cases',
                                                    start=1, end=40, start_from=30, unit='day',
                                                    fit_x0=(0.1, 1), formula='approx_conditional'))
    ...
    def test_mtbi_owid_arg_50_days_from_30(self):
        ...
        mtbis = np.array(Fitter.calculate_mtbis('Argentina', dataset='total_cases',
                                                start=1, end=50, start_from=30, unit='day',
                                                fit_x0=(0.1, 1), formula='approx_conditional'))
    ...
    def test_mtbi_owid_arg_50_days_from_40(self):
        ...
        mtbis = np.array(Fitter.calculate_mtbis('Argentina', dataset='total_cases',
                                                start=1, end=50, start_from=40, unit='day',
                                                fit_x0=(0.1, 1), formula='approx_conditional'))
